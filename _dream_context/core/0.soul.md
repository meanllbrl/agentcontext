...
dreamcontext is a TypeScript CLI tool published as an npm package that gives AI coding agents a structured, persistent brain across sessions. The core problem it solves is not that agents "forget" — it's that every session triggers a token-burning search spiral: the agent reads a flat memory file, finds it insufficient for a complex codebase, and burns 20K+ tokens in tool calls re-exploring structure it already knew. dreamcontext eliminates this by pre-loading structured context via a SessionStart hook (zero agent tool calls), organizing knowledge across purpose-built files mirroring brain regions (soul/identity, user/episodic, memory/semantic, knowledge/procedural, state/working), and consolidating knowledge during a RemSleep cycle that mirrors how the brain processes memory during sleep.

The tool ships with a SKILL.md (auto-loaded by Claude Code via `alwaysApply: true`) and three sub-agents: an initializer for bootstrapping, a rem-sleep agent for memory consolidation, and an explore agent for context-first codebase exploration. Seven Claude Code hooks are registered: SessionStart (context injection + sleep debt), Stop (transcript recording), SubagentStart (lightweight briefing for sub-agents), PreToolUse (blocks default Explorer when _dream_context/ exists), UserPromptSubmit (persistent sleep debt reminder on every user message), PostToolUse (auto-format + tsc check on JS/TS edits), PreCompact (saves compaction record before context compaction). Currently supports Claude Code only; multi-agent support is a future goal.

## Target User

Developers using Claude Code (and eventually other AI coding agents) who work on non-trivial projects where losing session context is painful. Also developers building on top of the dreamcontext system itself.

## Current Priority

Optional skill-packs Phase 2 CLI complete. `install-skill` now has `--packs` (interactive checkbox browser + direct names), `--skill` (individual sub-skill install), `--list`. Cross-pack dependency warnings, agent install, firebase reference copying. 490 tests passing. Rebrand from `agentcontext` to `dreamcontext` also complete (package, binary, GitHub, context dir). Next: Phase 3 of optional-skill-packs (official/community skills, user to supply sources), then Dashboard Phase 5 (accessibility audit, responsive layout, i18n tokens, bundle audit) before v0.2.0 release.

## Core Principles

- **Context should be cheap to load**: `dreamcontext snapshot` must be fast and produce a compact, high-signal output. No bloat.
...
- **Templates must ship with dist**: tsup `onSuccess` copies `src/templates` to `dist/templates`. If this breaks, `dreamcontext init` silently creates minimal stubs.
...
- **Single bin entry**: The package exposes one binary: `dreamcontext`. No sub-binaries.
...
- **Features are core, not scope creep**: Feature PRDs (with user stories, acceptance criteria, constraints) give the agent complete context when fixing bugs or building features. Solo developers don't use Jira/Linear — dreamcontext fills that gap intentionally.
...
- Do not add `.claude/` files, sub-agents, or skill configuration to this repo's own context. The skill and agents live in `skill/` and `agents/` for distribution — they are installed by `dreamcontext install-skill` into consuming projects, not this one.
- The project was renamed from `agentcontext` to `dreamcontext`. Any remaining references to `agentcontext` in source code, tests, or docs are bugs. The context directory is `_dream_context/` (not `_agent_context/`). The binary is `dreamcontext`. The GitHub repo is `meanllbrl/dreamcontext`.
