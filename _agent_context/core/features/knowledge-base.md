...
tags: ["architecture", "backend", "decisions"]
related_tasks: []
---
...
- **[2026-02-25]** Knowledge files live in `_agent_context/knowledge/` as flat `.md` files (no subdirectories). Tags provide the organizational structure instead of folders.
...
**Knowledge file location**: `_agent_context/knowledge/<slug>.md`
...
- The REM sleep agent (`agents/agentcontext-rem-sleep.md`) is the primary consumer of `knowledge create` — it extracts verbose detail from core files into knowledge files when they approach 200 lines. The main agent creates knowledge files for deep research completed during a session.
