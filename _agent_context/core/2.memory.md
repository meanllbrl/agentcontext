...
- **2 pre-existing flaky tests**: `tests/unit/id.test.ts` (`generates 8-char suffix after prefix`, nanoid(8) intermittently returns 5 chars) and a snapshot changelog integration test. 336 tests pass otherwise. Fix nanoid usage or mock before next publish.
