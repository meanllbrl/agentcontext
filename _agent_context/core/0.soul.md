...
Neuroscience-inspired memory system complete (8 phases, 394 tests all passing). 13 commands in `src/cli/index.ts` (added bookmark, trigger, transcript). transcript distill improved: full content, thinking blocks, subagent I/O, byte deltas, timestamp-based auto-filter tied to SleepHistoryEntry.consolidated_at. Dashboard Phase 5 (accessibility, responsive polish, i18n token extraction, bundle audit) is the next priority before v0.2.0 release.
...
- **13 commands total**: bookmark, trigger, transcript added in neuroscience session. Register new commands in `src/cli/index.ts` and add to SKILL.md command reference table.
