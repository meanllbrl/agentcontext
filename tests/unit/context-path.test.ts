...
describe('context-path', () => {
  ...
  describe('resolveContextRoot', () => {
    it('returns path when _agent_context/ exists in given dir', () => {
      const ctxDir = join(tmpDir, '_agent_context');
      mkdirSync(ctxDir);
      const result = resolveContextRoot(tmpDir);
      expect(result).toBe(ctxDir);
    });

    it('returns null when _agent_context/ does not exist', () => {
      const result = resolveContextRoot(tmpDir);
      expect(result).toBeNull();
    });

    it('walks up parent directories to find _agent_context/', () => {
      const ctxDir = join(tmpDir, '_agent_context');
      mkdirSync(ctxDir);
    ...
    it('finds _agent_context/ exactly at MAX_WALK_UP (5 levels)', () => {
      const ctxDir = join(tmpDir, '_agent_context');
      mkdirSync(ctxDir);
  ...
  describe('ensureContextRoot', () => {
    it('returns path when _agent_context/ exists', () => {
      const ctxDir = join(tmpDir, '_agent_context');
      mkdirSync(ctxDir);
      expect(ensureContextRoot(tmpDir)).toBe(ctxDir);
    });

    it('throws when _agent_context/ does not exist', () => {
      expect(() => ensureContextRoot(tmpDir)).toThrow(
        '_agent_context/ not found',
      );
    });

    it('error message suggests running init', () => {
      expect(() => ensureContextRoot(tmpDir)).toThrow('agentcontext init');
    });
  });

  describe('contextExists', () => {
    it('returns true when _agent_context/ exists', () => {
      mkdirSync(join(tmpDir, '_agent_context'));
      expect(contextExists(tmpDir)).toBe(true);
    });

    it('returns false when _agent_context/ does not exist', () => {
  ...
  describe('getInitPath', () => {
    it('returns _agent_context path in cwd', () => {
      const orig = process.cwd();
      try {
        process.chdir(tmpDir);
        expect(getInitPath()).toBe(join(tmpDir, '_agent_context'));
      } finally {
        process.chdir(orig);
      }
    });
